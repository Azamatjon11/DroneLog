#!/usr/bin/env python3
"""Task 1: frame-size classification from ULog-native IMU features."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Task 1: frame-size classification.")
...
def collect_dataset(args: argparse.Namespace, flights) -> tuple[np.ndarray, np.ndarray, np.ndarray, list[str]]:
    ...
    for index, record in enumerate(flights, start=1):
        ...
        for _start, _end, features in windows:
            ...
            labels.append(int(record.frame_mm))
...
def evaluate(X: np.ndarray, y: np.ndarray, groups: np.ndarray, requested_folds: int) -> pd.DataFrame:
    ...
    if folds != requested_folds:
        print(f"Using {folds} folds because grouped class counts limit the requested {requested_folds}.")
...
def main() -> None:
    ...
    args = parse_args()
    np.random.seed(SEED)
    print_startup("Task 1 - Frame-size classification")
    flights = find_flights(args.data, "frame")
    if not flights:
        raise SystemExit(f"No frame-size .ulg files found below {args.data}")
    if args.inspect:
    ...
    output = args.out / "task1_frame_results.csv"
